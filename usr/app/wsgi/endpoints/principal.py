...
from ..schema import SubjectPrincipalSet


class PrincipalEndpoint(sq.interfaces.http.Endpoint):
    ...
    A :class:`PrincipalEndpoint` validates the structure of the request headers,
    URL parameters, query parameters and entity prior to forwarding the
    request to its handler (controller).

    The handler function (e.g., :meth:`~PrincipalController.get()`) may,
    instead of a :class:`~sq.interfaces.http.Response` object, return a tuple
    or a dictionary. The :class:`PrincipalEndpoint` instance will interpret
